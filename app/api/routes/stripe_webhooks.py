...
#     """
#     Stripe Checkout Session totals:
#       - amount_total (preferred) is an integer in the smallest currency unit (cents for USD)
#       - fallback: total_details.amount_discount / amount_tax etc not needed; amount_total already final
#       - some older/edge flows might not include amount_total; try payment_intent.amount_received only if needed
#     """
...
# def _save_order_total_cents(db: Session, order_id: int, total_cents: int) -> None:
#     """
#     Save total_cents only if:
#       - incoming value is not None
#       - AND (orders.total_cents is NULL OR different)  (idempotent)
#     """
#     db.execute(
#         text(
#             """
...
#              where id = :oid
#                and (:tc is not null)
#                and (total_cents is null or total_cents <> :tc)
#             """
#         ),
#         {"oid": int(order_id), "tc": int(total_cents)},
#     )
...
#         # ✅ SAVE TOTAL (amount_total) into orders.total_cents
#         total_cents = _extract_total_cents_from_session(obj)
#         if total_cents is not None:
#             try:
#                 _save_order_total_cents(db, int(oid), int(total_cents))
#                 db.commit()
#                 _log("saved total_cents", total_cents, "for order", oid)
#             except Exception as e:
#                 db.rollback()
#                 _log("warn: failed saving total_cents", "order", oid, type(e).__name__, str(e))
#                 # don't block fulfillment if total save fails
