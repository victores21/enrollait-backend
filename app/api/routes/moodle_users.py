...
from app.core.db import get_db
from app.core.tenant import get_tenant_id_from_request  # ✅ new tenant methodology
from app.services.moodle import MoodleClient, MoodleError
...
@router.post("/integrations/moodle/users/ensure")
async def ensure_moodle_user(
    payload: CreateMoodleUserPayload,
    ...
    # 1) First check if user exists in Moodle (by email)
    try:
        data = await moodle.call(
            "core_user_get_users",
            **{
        ...
        return {"ok": False, "message": f"Moodle error (search): {str(e)}", "tenant_id": tenant_id}
    except Exception as e:
        return {"ok": False, "message": f"Failed (search): {type(e).__name__}: {str(e)}", "tenant_id": tenant_id}

    # 2) If exists, store mapping and return
    if users:
        moodle_user_id = int(users[0]["id"])
    ...
    try:
        ...
        return {"ok": False, "message": f"Moodle error (create): {str(e)}", "tenant_id": tenant_id}
    except Exception as e:
        return {"ok": False, "message": f"Failed (create): {type(e).__name__}: {str(e)}", "tenant_id": tenant_id}
